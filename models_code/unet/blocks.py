...
def conv_block(inputs, filters, activation='relu', dropout_rate=0.0):
...
def encoder_block(inputs, filters, activation='relu', dropout_rate=0.0):
    x = conv_block(inputs, filters, activation=activation, dropout_rate=dropout_rate)
    p = layers.MaxPooling2D(pool_size=(2, 2))(x)
    return x, p

def decoder_block(inputs, skip_features, filters, activation='relu', dropout_rate=0.0):
